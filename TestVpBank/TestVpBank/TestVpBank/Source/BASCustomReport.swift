...
public class BASCustomReport: UIView {
   ...
   fileprivate func registerCells() {
        ...
        longPressGesture = UILongPressGestureRecognizer(target: self, action: #selector(BASCustomReport.handleLongGesture(gesture:)))
    
//        colReportSummary?.addGestureRecognizer(longPressGesture)
    ...
    @objc func handleLongGesture(gesture: UILongPressGestureRecognizer)
    {
        ...
        case UIGestureRecognizerState.began:
            guard let selectedIndexPath = colReportSummary?.indexPathForItem(at: gesture.location(in: colReportSummary)) else {
                break
            }
            colReportSummary?.beginInteractiveMovementForItem(at: selectedIndexPath)
        case UIGestureRecognizerState.changed:
            colReportSummary?.updateInteractiveMovementTargetPosition(gesture.location(in: gesture.view!))
        case UIGestureRecognizerState.ended:
            colReportSummary?.endInteractiveMovement()
        default:
            colReportSummary?.cancelInteractiveMovement()
        }
    }
...
//        let item = self.arrReportSummaryDict[indexPath.row]
